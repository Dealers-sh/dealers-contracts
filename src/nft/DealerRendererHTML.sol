...
contract DealerRendererHTML is IDealerRendererHTML, Ownable {
    ...
    function getHTML(uint256 tokenId, string memory) external view override returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                '<meta name="viewport" content="width=device-width,initial-scale=1">' "<title>Dealer #",
                id,
    ...
    function _loaderScript(uint256 tokenId) private view returns (bytes memory) {
        return abi.encodePacked(
            _configBlock(tokenId), _rpcHelpers(), _abiHelpers(), _loadSvgFn(), _loadAppJsFn(), _mainFn()
        );
    }
    ...
    function _configBlock(uint256 tokenId) private view returns (bytes memory) {
        return abi.encodePacked(
            ...
            rpcUrl,
            '",S="',
            svgRendererAddress.toHexStringChecksummed(),
            '",F="',
            address(fileStore).toHexStringChecksummed(),
            '",N="',
            dealerGzipFilename,
            '";'
        );
    ...
    function _rpcHelpers() private pure returns (bytes memory) {
        return bytes(
            "async function rpc(m,p){" 'var r=await fetch(R,{method:"POST",headers:{"Content-Type":"application/json"},'
    ...
    function _loadSvgFn() private pure returns (bytes memory) {
        ...
        return bytes(
            "async function loadSVG(){" 'setStatus("fetching dealer");' 'var d="0xbe985ac9"+p64(T);'
            "var r=await ec(S,d);" "return ds(r.slice(2))}"
        );
    }
    ...
    function _loadAppJsFn() private pure returns (bytes memory) {
        ...
        return bytes(
            "async function loadAppJs(){" 'setStatus("fetching pointers");' 'var d="0xe0876aa8"+es(N);'
            "var r=await ec(F,d);" 'var h=r.startsWith("0x")?r.slice(2):r;'
            ...
            "var so=parseInt(h.substr(base+64,64),16)*2;" "var sa=base+so;" "var sl=parseInt(h.substr(sa,64),16);"
            "var chunks=[];" 'setStatus("reading bytecode");' "for(var i=0;i<sl;i++){" "var off=sa+64+i*192;"
            'var ptr="0x"+h.substr(off+24,40);' "var st=parseInt(h.substr(off+64,64),16);"
            "var en=parseInt(h.substr(off+128,64),16);" 'var code=await rpc("eth_getCode",[ptr,"latest"]);'
            "var raw=hx(code);" "chunks.push(raw.slice(st,en))}" 'setStatus("decompressing");'
            "var total=chunks.reduce(function(a,c){return a+c.length},0);" "var merged=new Uint8Array(total);"
            "var pos=0;chunks.forEach(function(c){merged.set(c,pos);pos+=c.length});"
            'var b64="";for(var i=0;i<merged.length;i++)b64+=String.fromCharCode(merged[i]);' "var gz=atob(b64);"
            "var ga=new Uint8Array(gz.length);" "for(var i=0;i<gz.length;i++)ga[i]=gz.charCodeAt(i);"
    ...
    function _mainFn() private pure returns (bytes memory) {
        return bytes(
            ...
            'function failStatus(){if(SS){SS.textContent=stage+" failed";SS.className="err"}}' "(async function(){"
            'BB=document.getElementById("b");' 'SS=document.getElementById("s");'
            "BI=setInterval(function(){BB.textContent=FR[(fi++)%FR.length]},80);" 'setStatus("rpc check");'
            'try{await rpc("eth_blockNumber",[])}' 'catch(e){console.error("RPC failed:",e);failStatus();return}'
            "var svg;" "try{svg=await loadSVG()}" 'catch(e){console.error("SVG load failed:",e);failStatus()}' "var js;"
            "try{js=await loadAppJs()}" 'catch(e){console.error("App load failed:",e);failStatus()}' "if(js){"
            'setStatus("rendering");' "await new Promise(function(r){setTimeout(r,200)});" "clearInterval(BI);"
            'document.body.innerHTML=(svg||"")+\'<div id="dealer-ui" style="position:absolute;inset:0"></div>\';'
            'var e=document.createElement("script");e.textContent=js;document.head.appendChild(e)' "}else if(svg){"
            'setStatus("rendering");' "await new Promise(function(r){setTimeout(r,200)});" "clearInterval(BI);"
