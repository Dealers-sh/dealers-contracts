...
/**
 * @title DealersClaims - Achievement & admin reward claims
 *
 * █▀▄ █▀▀ ▄▀█ █░░ █▀▀ █▀█ █▀ ░ █▀ █░█
 * █▄▀ ██▄ █▀█ █▄▄ ██▄ █▀▄ ▄█ ▄ ▄█ █▀█
 *
 ...
 * @author Berny0x
 */
contract DealersClaims is ReentrancyGuard, Ownable {
    // =============================================================
    //                            ENUMS
    // =============================================================
    ...
    function claimAchievement(uint256 tokenId, uint256 achievementId) external nonReentrant {
        if (dealersNFT.ownerOf(tokenId) != msg.sender) revert NotTokenOwner();
    ...
    function _grantReward(uint256 tokenId, uint8 rewardType, uint256 rewardId, uint256 amount) internal {
        if (rewardType == uint8(RewardType.REPUTATION)) {
            dealersCore.updateReputation(tokenId, int256(amount));
        } else if (rewardType == uint8(RewardType.CASH)) {
            dealersCore.addCash(tokenId, amount);
    ...
    function setDealersCore(address _core) external onlyOwner {
        ...
        dealersCore = IDealersCore(_core);
    }

    function setDealersNFT(address _nft) external onlyOwner {
        if (_nft == address(0)) revert InvalidAddress();
        dealersNFT = IERC721Minimal(_nft);
    }
