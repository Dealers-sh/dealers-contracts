...
import {DealersExeCore} from "./DealersExeCore.sol";
...
contract DealersExeActions is ReentrancyGuard, Ownable {
        ...
        onlyDealerOwner(tokenId)
    {
        ...
        if (exitingBlackMarket) {
            ...
            if (!areaRegistry.isValidArea(destinationArea)) revert DealersExeCore.InvalidArea();
            if (areaRegistry.isJail(destinationArea)) revert DealersExeCore.CannotEnterJail();
            uint256 minRep = areaRegistry.getMinReputation(destinationArea);
            if (minRep > 0 && gs.totalReputation < minRep) revert DealersExeCore.InsufficientReputation();
        }
