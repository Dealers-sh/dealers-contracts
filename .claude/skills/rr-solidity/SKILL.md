...
## Abstract Chain / zkSync Deployment

This project targets Abstract Chain (zkSync-based L2). Abstract supports two execution modes:

### Dual-Mode Architecture

| Contract Type | Execution Mode | Flag | Use Case |
|--------------|----------------|------|----------|
| Game logic contracts | Native zkSync | `--zksync` | Gas-efficient state mutations |
| Renderer contracts | EVM Interpreter | (none) | EXTCODECOPY/SSTORE2 compatibility |
...
**Game contracts** (Core, NFT, PVE, PVP, Boosts):
```bash
forge script script/DeployGame.s.sol --zksync --rpc-url https://api.testnet.abs.xyz --broadcast
```

**Renderer contracts** (SVG, HTML - use EXTCODECOPY via SSTORE2):
```bash
...
When deploying EVM bytecode contracts on Abstract:
- `DELEGATECALL` between EVM and native zkSync contracts will revert
...
**zkSync/Abstract Chain:**
```bash
forge build --zksync                           # Build for native zkSync
forge test --zksync                            # Test in zkSync mode
forge script script/Deploy.s.sol --zksync --broadcast  # Deploy native zkSync
```
...
### Project Rules
- `.claude/rules/forge.md` - Abstract Chain build/deployment rules
