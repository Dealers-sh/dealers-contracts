...
import {IDealersChatGate} from "../../src/social/IDealersChatGate.sol";
...
contract MockGate is IDealersChatGate {
...
contract RejectAllGate is IDealersChatGate {
    function canPost(uint16, uint8) external pure override returns (bool) {
        return false;
    }
}

contract DealersChatFactoryTest is Test {
    DealersChatFactory public factory;
    ...
    function setUp() public {
        ...
        factory = new DealersChatFactory(address(mockNft));

        mockNft.setOwner(TOKEN_ALICE, alice);
        mockNft.setOwner(TOKEN_BOB, bob);

        factory.createRoom(DealersChatFactory.RoomType.WORLD, 0, address(0));
        worldKey = factory.roomKey(DealersChatFactory.RoomType.WORLD, 0);
    }
    ...
    function test_createRoom_deploysAndRegisters() public view {
        address room = factory.getRoomAddress(DealersChatFactory.RoomType.WORLD, 0);
        assertTrue(room != address(0));
        assertEq(IDealersChatRoom(room).factory(), address(factory));
    }

    function test_createRoom_storesGateAndRoomId() public {
        factory.createRoom(DealersChatFactory.RoomType.AREA, 3, address(mockGate));
        bytes32 key = factory.roomKey(DealersChatFactory.RoomType.AREA, 3);
    ...
    function test_createRoom_revertsDuplicate() public {
        vm.expectRevert(DealersChatFactory.RoomAlreadyExists.selector);
        factory.createRoom(DealersChatFactory.RoomType.WORLD, 0, address(0));
    }

    function test_createRoom_revertsUnauthorized() public {
        vm.prank(alice);
        vm.expectRevert(DealersChatFactory.NotAuthorized.selector);
        factory.createRoom(DealersChatFactory.RoomType.AREA, 1, address(0));
    }

    function test_createRoom_multipleTypes() public {
        factory.createRoom(DealersChatFactory.RoomType.AREA, 1, address(0));
        factory.createRoom(DealersChatFactory.RoomType.AREA, 2, address(0));

        address world = factory.getRoomAddress(DealersChatFactory.RoomType.WORLD, 0);
        address area1 = factory.getRoomAddress(DealersChatFactory.RoomType.AREA, 1);
        address area2 = factory.getRoomAddress(DealersChatFactory.RoomType.AREA, 2);
    ...
    function test_authorizedCreator_canCreateRoom() public {
        ...
        address room = factory.createRoom(DealersChatFactory.RoomType.GANG, 1, address(mockGate));
        assertTrue(room != address(0));
    }
    ...
    function test_authorizedCreator_revokedCannotCreate() public {
        ...
        vm.prank(gangContract);
        vm.expectRevert(DealersChatFactory.NotAuthorized.selector);
        factory.createRoom(DealersChatFactory.RoomType.GANG, 1, address(0));
    }
    ...
    function test_authorizeContract_revertsZeroAddress() public {
        vm.expectRevert(DealersChatFactory.InvalidAddress.selector);
    ...
    function test_postMessage_success() public {
        ...
        address room = factory.getRoomAddress(DealersChatFactory.RoomType.WORLD, 0);
        IDealersChatRoom.Message[] memory msgs = IDealersChatRoom(room).getLatestMessages(1);
    ...
    function test_postMessage_revertsRoomDoesNotExist() public {
        ...
        vm.prank(alice);
        vm.expectRevert(DealersChatFactory.RoomDoesNotExist.selector);
        factory.postMessage(fakeKey, TOKEN_ALICE, "hello");
    }

    function test_postMessage_revertsNotTokenOwner() public {
        vm.prank(bob);
        vm.expectRevert(DealersChatFactory.NotTokenOwner.selector);
    ...
    function test_postMessage_revertsBlocked() public {
        ...
        vm.prank(alice);
        vm.expectRevert(DealersChatFactory.DealerIsBlocked.selector);
    ...
    function test_postMessage_revertsCooldown() public {
        ...
        vm.prank(alice);
        vm.expectRevert(DealersChatFactory.CooldownActive.selector);
    ...
    function test_postMessage_succeedsAfterCooldown() public {
        ...
        address room = factory.getRoomAddress(DealersChatFactory.RoomType.WORLD, 0);
        assertEq(IDealersChatRoom(room).getMessageCount(), 2);
    }
    ...
    function test_postMessage_revertsMessageTooLong() public {
        ...
        vm.prank(alice);
        vm.expectRevert(DealersChatFactory.MessageTooLong.selector);
        factory.postMessage(worldKey, TOKEN_ALICE, string(longMsg));
    }

    function test_postMessage_revertsMessageEmpty() public {
        vm.prank(alice);
        vm.expectRevert(DealersChatFactory.MessageEmpty.selector);
    ...
    function test_postMessage_emitsEvent() public {
        ...
        emit DealersChatFactory.MessageRouted(worldKey, TOKEN_ALICE);
    ...
    function test_postMessage_gatedRoom_allowedPasses() public {
        factory.createRoom(DealersChatFactory.RoomType.AREA, 5, address(mockGate));
        bytes32 areaKey = factory.roomKey(DealersChatFactory.RoomType.AREA, 5);
        ...
        address room = factory.getRoomAddress(DealersChatFactory.RoomType.AREA, 5);
        assertEq(IDealersChatRoom(room).getMessageCount(), 1);
    }
    ...
    function test_postMessage_gatedRoom_deniedReverts() public {
        ...
        vm.prank(alice);
        vm.expectRevert(DealersChatFactory.GateCheckFailed.selector);
    ...
    function test_postMessage_rejectAllGate() public {
        ...
        factory.createRoom(DealersChatFactory.RoomType.GANG, 1, address(rejectGate));
        bytes32 gangKey = factory.roomKey(DealersChatFactory.RoomType.GANG, 1);
    ...
    function test_roomKey_deterministic() public view {
        bytes32 expected = keccak256(abi.encodePacked(uint8(DealersChatFactory.RoomType.WORLD), uint8(0)));
        assertEq(factory.roomKey(DealersChatFactory.RoomType.WORLD, 0), expected);
    }
    ...
    function test_getRoomAddress_returnsZeroForNonexistent() public view {
        address room = factory.getRoomAddress(DealersChatFactory.RoomType.GANG, 99);
        assertEq(room, address(0));
    }

    function test_getRoomInfo_returnsAllFields() public {
        factory.createRoom(DealersChatFactory.RoomType.AREA, 7, address(mockGate));
        bytes32 key = factory.roomKey(DealersChatFactory.RoomType.AREA, 7);
    ...
    function test_constructor_revertsZeroAddress() public {
        vm.expectRevert(DealersChatFactory.InvalidAddress.selector);
        new DealersChatFactory(address(0));
    }
    ...
    function test_cooldown_sharedAcrossRooms() public {
        factory.createRoom(DealersChatFactory.RoomType.AREA, 1, address(0));
        bytes32 areaKey = factory.roomKey(DealersChatFactory.RoomType.AREA, 1);
