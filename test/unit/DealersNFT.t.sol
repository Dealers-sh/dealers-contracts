...
contract DealersNFTTest is BaseTest {
    ...
    function test_mintPublic_revertWhenDisabled() public {
        assertEq(uint8(nft.mintStatus()), uint8(DealersNFT.MintStatus.DISABLED));

        vm.prank(player1);
        vm.expectRevert(DealersNFT.NotPublicMint.selector);
    ...
    function test_setMintStatus_changesStage() public {
        ...
        vm.prank(owner);
        nft.setMintStatus(DealersNFT.MintStatus.FAMILY);
        assertEq(uint8(nft.mintStatus()), uint8(DealersNFT.MintStatus.FAMILY));

        vm.prank(owner);
        nft.setMintStatus(DealersNFT.MintStatus.WHITELIST);
        assertEq(uint8(nft.mintStatus()), uint8(DealersNFT.MintStatus.WHITELIST));

        vm.prank(owner);
        nft.setMintStatus(DealersNFT.MintStatus.PUBLIC);
        assertEq(uint8(nft.mintStatus()), uint8(DealersNFT.MintStatus.PUBLIC));
    }
    ...
    function test_mintFamily_revertInvalidProof() public {
        ...
        vm.prank(player1);
        vm.expectRevert(DealersNFT.InvalidMerkleProof.selector);
    ...
    function test_mintFamily_revertExceedsAllocation() public {
        ...
        vm.prank(player1);
        vm.expectRevert(DealersNFT.ExceedsAllocation.selector);
    ...
    function test_mintFamily_revertMerkleRootNotSet() public {
        ...
        freshNft.setMintStatus(DealersNFT.MintStatus.FAMILY);

        bytes32[] memory proof = new bytes32[](0);

        vm.prank(player1);
        vm.expectRevert(DealersNFT.MerkleRootNotSet.selector);
    ...
    function test_mint_emitsDealerInitialized() public {
        ...
        emit DealersNFT.DealerInitialized(nextTokenId, player1);
    ...
    function test_mint_withoutCore_noInitialization() public {
        DealersNFT nftNoCore = new DealersNFT(devWallet);

        vm.prank(address(this));
        nftNoCore.setMintStatus(DealersNFT.MintStatus.PUBLIC);
        ...
        assertEq(nftNoCore.dealersCore(), address(0));
    }
    ...
    function test_withdraw_allToOwner() public {
        ...
        freshNft.setMintStatus(DealersNFT.MintStatus.PUBLIC);
